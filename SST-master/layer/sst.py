...
def add_extras(cfg, i, batch_norm=True):
    ...
    layers+= [conv2d2, nn.ReLU(inplace=True), nn.BatchNorm2d(512)]#, nn.Dropout(p=0.1)]
    flag = False
    in_channels = "S"
    ...
    layers += [conv2d3, nn.ReLU(inplace=True), nn.BatchNorm2d(128)]#, nn.Dropout(p=0.1)]
    flag = True
    in_channels = 128

    #v='S'
    conv2d4 = nn.Conv2d(in_channels, 256, kernel_size=3, stride=2, padding=1)
    layers+= [conv2d4, nn.ReLU(inplace=True), nn.BatchNorm2d(256)]#, nn.Dropout(p=0.1)]
    flag = False
    in_channels = "S"
    ...
    layers += [conv2d5, nn.ReLU(inplace=True), nn.BatchNorm2d(128)]#, nn.Dropout(p=0.1)]
    flag = True
    in_channels = 128

    #v=256
    conv2d6 = nn.Conv2d(in_channels, 256, kernel_size=3)
    layers += [conv2d6, nn.ReLU(inplace=True), nn.BatchNorm2d(256)]#, nn.Dropout(p=0.1)]
    flag = False
    in_channels = 256

    #v=128
    conv2d7 = nn.Conv2d(in_channels, 128, kernel_size=1)
    layers += [conv2d7, nn.ReLU(inplace=True), nn.BatchNorm2d(128)]#, nn.Dropout(p=0.1)]
    flag = True
    in_channels = 128

    #v=256
    conv2d8 = nn.Conv2d(in_channels, 256, kernel_size=3)
    layers += [conv2d8, nn.ReLU(inplace=True), nn.BatchNorm2d(256)]#, nn.Dropout(p=0.1)]
    flag = False
    in_channels = 256

    #v=128
    conv2d9 = nn.Conv2d(in_channels, 128, kernel_size=1)
    layers += [conv2d9, nn.ReLU(inplace=True), nn.BatchNorm2d(128)]#, nn.Dropout(p=0.1)]
    flag = True
    in_channels = 128

    #v="S"
    conv2d10 = nn.Conv2d(in_channels, 256, kernel_size=3, stride=2, padding=1)
    layers+= [conv2d10, nn.ReLU(inplace=True), nn.BatchNorm2d(256)]#, nn.Dropout(p=0.1)]
    flag = False
    in_channels = "S"
    ...
    layers += [conv2d11, nn.ReLU(inplace=True), nn.BatchNorm2d(128)]#, nn.Dropout(p=0.1)]
    flag = True
    in_channels = 128
...
def selector(vgg, extra_layers, batch_normal=True):
    ...
    if batch_normal:
        ...
        #for k, v in enumerate(extra_layers[4::8], 4):
